...
if __name__ == "__main__": raise SystemExit("This file is not meant to be run directly. Please run the main script.")
...
def single_select(title: str, options: list[str], allow_back: bool = True, toast: tuple[str,str] = None) -> int:
    ...
    while True:
        ...
        if total_pages > 1:
            ...
            elif COLOR_ENABLED: print_colored("P. Previous Page", 'gray')
            if page < total_pages - 1: print("N. Next Page")
            elif COLOR_ENABLED: print_colored("N. Next Page", 'gray')
